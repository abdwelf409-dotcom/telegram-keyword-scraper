...
# ========== كلمات الإعلانات (لتجنب إعلانات الكبار والشركات) ==========
AD_PATTERNS = [
...
# ========== الكلمات المفتاحية الشاملة والمحدثة ==========
KEYWORDS_SET = {
    ...
    # الخدمات والتخصصات الأكاديمية
    'خدمات طلابية', 'خدمات أكاديمية', 'خدمات جامعية', 'مساعدة جامعية', 
    'دعم أكاديمي', 'دعم طلابي', 'حل واجبات', 'حل تكاليف', 'حل أسايمنت', 
    'حل Assignment', 'حل TMA', 'حل EMA', 'حل كويزات', 'حل اختبارات', 
    'حل ميد', 'حل فاينل', 'كتابة بحوث', 'إعداد بحوث', 'كتابة أبحاث', 'بحث جامعي', 
    'مشروع تخرج', 'تقرير تدريب', 'تدريب تعاوني', 'تدريب ميداني', 'تلخيص', 
    'ملخصات', 'عروض بوربوينت', 'PowerPoint', 'تصميم عروض', 'تصميم إنفوجرافيك', 
    'تصميم شعارات', 'تصميم بوستات', 'تصميم سوشيال ميديا', 'تصميم هوية', 
    'ترجمة أكاديمية', 'تدقيق لغوي', 'إعادة صياغة', 'تنسيق أبحاث', 'توثيق APA', 
    'توثيق Harvard', 'تحليل SPSS', 'Excel', 'Word', 'PDF', 'تحويل PDF إلى Word', 
    'إدخال بيانات', 'كتابة محتوى', 'كتابة مقالات', 'كتابة خطابات', 'سيرة ذاتية', 
    'Cover Letter', 'Portfolio', 'ملف إنجاز', 'مشاريع برمجة', 'تصميم مواقع', 
    'تطبيقات', 'قواعد بيانات', 'رسائل ماجستير', 'رسائل دكتوراه', 'خطة بحث', 
    'Proposal', 'استبيان', 'طباعة أبحاث', 'تفريغ صوتي', 'إجازة مرضية', 'اجازة مرضية', 
    'سكليف', 'تقرير طبي', 'عذر طبي', 'عذر غياب', 'خطاب اعتذار', 'رفع الأعذار', 
    'تسجيل مقررات', 'حذف وإضافة', 'تأجيل دراسة', 'اعتذار عن فصل'
}
...
def is_ad(text):
    """فحص متطور لتجاهل الإعلانات ورسائل الشركات أو المكاتب الكبيرة"""
    if not text:
        return False
    text_lower = text.lower()
    
    # 1. مطابقة أنماط الكلمات الإعلانية
    for pattern in AD_PATTERNS:
        if re.search(pattern, text_lower):
            return True
            
    # 2. فحص الرسائل الطويلة جداً التي تحتوي على عروض أسعار أو ترويج مكاتب
    if len(text) > 600:
        ad_indicators = ['عرض', 'سعر', 'خصم', 'للبيع', 'اشتري', 'متجر', 'توصيل', 'ضمان', 'معتمد', 'خدماتنا', 'للتواصل']
        count = sum(1 for w in ad_indicators if w in text_lower)
        if count >= 2:
            return True
...
def check_keywords_fast(text):
    """بحث فوري - O(1) للكلمات المفتاحية"""
    if not text:
        return None
    text_lower = text.lower()
...
async def main():
    ...
    async def handler(event):
        ...
        # فلتر الإعلانات المحدث لتجنب إعلانات الكبار
        if is_ad(text):
            return
        
        # البحث الفوري عن الكلمات المفتاحية للطلاب
