...
# ========== دالة توحيد وتطبيع النصوص لرفع الحساسية ==========
def normalize_text(text):
    if not text:
        return ""
    text = text.lower()
    # إزالة الأشكال المختلفة للهمزات والمدود
...
# ========== الكلمات المفتاحية للطلبات (التي يبحث عنها الطلاب والأهالي) ==========
KEYWORDS_SET = {
    # الكلمات المفتاحية الجديدة الخاصة بتأسيس وتطوير الأطفال
    'انا محتاج تاسيس لا اولادي', 'انا محتاج تاسيس لاولادي', 'محتاج تاسيس لاولادي',
    'محتاجة تاسيس لاولادي', 'محتاج تاسيس لأولادي', 'محتاجة تاسيس لأولادي',
    'لبنتي', 'لابني', 'ابغي تاسيس', 'ابي تاسيس', 'محتاج تاسيس', 'محتاجة تاسيس',
    'ابني مايعرف يقرا', 'ابني مايعرف يقراء', 'ابني مايعرف يقراا', 'ابني ما يعرف يقرا',
    'بنتي ما تعرف تقرا', 'بنتي ما تعرف تقراء', 'بنتي مايعرف تقرا',
    'ماذا اعمل لابني دورات', 'ماذا اعمل لبنتي دورات', 'دورات لابني', 'دورات لبنتي',
    'كيف اطور ابني بالقراءه', 'كيف اطور ابني بالقراءة', 'كيف اطور بنتي بالقراءة',
    'كيف اطور ابني في القراءة', 'تاسيس اطفال', 'تاسيس رياضيات', 'تاسيس انجليزي', 'تاسيس عربي',
...
# pre-normalize الكلمات المفتاحية لسرعة ودقة المطابقة
NORMALIZED_KEYWORDS = {kw: normalize_text(kw) for kw in KEYWORDS_SET}

# ========== تصنيفات الرسائل ==========
CATEGORIES = {
    'تأسيس وأطفال': {'تاسيس', 'اطفال', 'اولادي', 'ابني', 'بنتي', 'يقرا', 'قراءه', 'دورات'},
...
CATEGORY_COLORS = {
    'تأسيس وأطفال': '👶', 'اختبار': '🔴', 'واجب': '🟢', 'مشروع': '🔵',
    'بحث': '🟣', 'تلخيص': '🟡', 'ترجمة': '🟠',
    'بوربوينت': '⚫', 'تصميم': '🔘', 'cv': '🟤',
    'برمجة': '⚪', 'سكليف': '🩺', 'عام': '📌',
}
...
def get_category(keyword):
    ...
    for cat, words in CATEGORIES.items():
        if cat == 'عام':
            continue
        for w in words:
            if normalize_text(w) in kw_norm:
                return cat
    return 'عام'

def check_keywords_fast(text):
    """فحص شديد الحساسية للمطابقة التامة بغض النظر عن الحركات أو نوع الإملاء"""
    if not text:
        return None
...
# ========== الحدث الرئيسي للبوت ==========
async def main():
    print("🚀 البوت يعمل والفلتر الحاد للإعلانات الكبيرة نشط مع الحساسية المرتفعة...")
    await client.start()
    ...
    async def handler(event):
        ...
        # البحث الفائق الحساسية عن الكلمات المفتاحية
        ...
        if keyword:
            ...
            try:
                ...
                print("✅ [طلب حقيقي] " + full_name + " | " + keyword)
